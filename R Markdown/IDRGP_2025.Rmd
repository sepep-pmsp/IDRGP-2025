...
analise_acao_reg <- acao_reg %>%
  ...
  filter(n_distinct(TIPO_REGIONALIZAÇÃO) == 2) %>%
  mutate(total = sum(valor, na.rm = TRUE)) %>%
  mutate(percentual = round(valor / total, 2)) %>% 
  ungroup()
#verificanco as inconsistências de regionalização por proj/ativ ao longo dos anos
analise_acao_reg<- analise_acao_reg %>% 
  group_by(CÓDIGO_PROJ_ATIV, TIPO_REGIONALIZAÇÃO) %>%
filter(n_distinct(percentual) > 1)
#reumo (considera apenas variaçõea maiores de 5%)
...
tabela_inconsistencias %>% 
  ...
  group_by(CÓDIGO_PROJ_ATIV) %>% summarise(sum(valor))
```

A despeito de haver muitos casos NA em 2024, a tabela mostra que não há uma variação tão grande entre os casos regionalizados e não regionalizados na maioria das ações. Também não há tantas ações que se repetem com diferenças no perfil de regionalização ao longo dos anos. 
...
* *4113-Sistema Municipal de Regulação, Controle, Avaliação e Auditoria do SUS* não foi regionalizada em 2022, mas chegou a ser 100% regionalizada em 2025. Este é o caso de "inconsistência" (variação) que envolve maior montante de recursos.
* Outros casos de ampliação da regionalização do dado ao longo dos anos: *2635-Serviço de Moradia Transitória* e *6669-Educação Ambiental*.
