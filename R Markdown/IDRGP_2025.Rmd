...
if (!dir.exists(pasta_dados)) {
  stop("Pasta `pasta_dados` não existe: ", pasta_dados)
}
...
O Índice de Alocação Orçamentária Esperada - AOE ou IDRGP Alvo é o índice de referência que indica qual proporção dos gastos públicos deveria ser destinada a cada subprefeitura. Nesta análise utilizaremos a atualização da cesta de indicadores calculada em 2025 e nomeada como Índice de Vulnerabilidade Urbana (IVU) pela SEPLAN/CPMA. Esta é a mesma base utilizada no relatório do IDRGP 2024.
...
# Checa arquivos antes (erro rápido e claro)
faltando <- arquivos[!file.exists(arquivos)]
if (length(faltando) > 0) {
  stop("Arquivos não encontrados:\n- ", paste(faltando, collapse = "\n- "))
}
...
# (Opcional, mas útil) expõe no ambiente como df_2022... e df_agr_2022...
list2env(setNames(dfs,  paste0("df_", anos)), envir = .GlobalEnv)
list2env(setNames(agrs, paste0("df_agr_", anos)), envir = .GlobalEnv)

# Revisão automática no final do chunk
resumo_anos <- tibble::tibble(
  ano = anos,
  n_linhas = purrr::map_int(dfs, nrow),
  n_subpref = purrr::map_int(agrs, ~ dplyr::n_distinct(.x$subprefeitura)),
  valor_total = purrr::map_dbl(agrs, ~ sum(.x$valor, na.rm = TRUE))
)

knitr::kable(
  resumo_anos |>
    dplyr::mutate(valor_total = scales::label_number(
...
**Metodologia:** O teste de **Kolmogorov-Smirnov** verifica a normalidade das diferenças pareadas. Conforme o resultado, se maior de 0,05, opta-se pelo **teste t pareado** (paramétrico) ou, se menor de 0,05, **Wilcoxon** (não paramétrico). 

Com o teste que analisa a distância entre as distribuições realizado, utiliza-se o intervalo de confiança (95%) para classificar cada subprefeitura em Acima, Abaixo ou Dentro da Alocação Esperada (ou do IDRGP Alvo).
...
```
****Incluir Teste estatístico no chunk acima****
...
```{r grafico3, echo=FALSE, fig.width=14, fig.height=12}
