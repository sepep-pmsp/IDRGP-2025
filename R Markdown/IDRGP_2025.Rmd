...
#O objetivo aqui é entender se o gasto real é estatisticamente diferente do gasto esperado
...
#testar normalidade para entender se a distribuição segue a normal  
...
# Adotou-se uma regra empírica de ±30% como critério operacional de variação relevante, complementarmente validada por testes estatísticos de hipótese.
  df_teste <- df_teste |>
    mutate(
...
#pipeline completo (dados → integração → indicador → teste → classificação)
```
