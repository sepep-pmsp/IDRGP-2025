...
- Uma tablea por sub com a distribuição dos maiores gastos por órgão
...
```{r}
# Segunda tabela
gasto_orgao_reg <- rbind(df_2022, df_2023, df_2024, df_2025) %>% 
  group_by(subprefeitura, sigla_orgao) %>% 
  summarise(valor = sum(valor_detalhamento_acao, na.rm = TRUE), .groups = "drop") %>% 
  group_by(subprefeitura) %>% 
  mutate(
    total_subpref = sum(valor),
    percentual = ((valor / total_subpref)*100),
