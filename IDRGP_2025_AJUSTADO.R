...
#   09. GRÁFICOS       — 4 gráficos (IDRGP %, valores R$, dumbbell ciclo)
...
# ---------------------------------------------------------------------------
# AJUSTE [1/4]: Sigla do Itaim Paulista corrigida de "TP" para "IT"
# O shapefile usa a sigla "IT" para Itaim Paulista. O valor "TP" causava
# falha no left_join, deixando o polígono com subprefeitura = NA e
# propagando NAs para mapas 2022–2024 e para os gráficos de 2025.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AJUSTE [2/4]: Checagem de siglas sem correspondência em df_siglas
# Após o join, qualquer sigla presente no shapefile mas ausente em df_siglas
# resultará em subprefeitura = NA. Este bloco detecta silenciosamente
# esse tipo de problema para evitar reincidência.
# ---------------------------------------------------------------------------
siglas_sem_match <- subprefeitura_sf |>
  st_drop_geometry() |>
  filter(is.na(subprefeitura)) |>
  select(sigla)

if (nrow(siglas_sem_match) > 0) {
  warning(
    "ATENÇÃO: ", nrow(siglas_sem_match),
    " sigla(s) do shapefile sem correspondência em df_siglas:\n",
    paste(siglas_sem_match$sigla, collapse = ", "),
    "\nAdicione a(s) sigla(s) à tabela df_siglas antes de prosseguir."
  )
} else {
  cat("✅ Todas as siglas do shapefile têm correspondência em df_siglas.\n")
}
...
#      Teste t pareado ou Wilcoxon pareado conforme normalidade das diferenças.
...
# ---------------------------------------------------------------------------
# AJUSTE [3/4]: Filtro defensivo de NA no grafico1
# Com a correção upstream da sigla do Itaim (Ajuste 1), esta linha não
# deve mais ter efeito prático. Mantida como proteção para futuros dados.
# ---------------------------------------------------------------------------

grafico1 <- df_integrado |>
  st_drop_geometry() |>
  filter(!is.na(subprefeitura), !is.na(idrgp_real), !is.na(status_estatistico)) |>
  ggplot(aes(y = reorder(subprefeitura, idrgp_alvo))) +
...
# ---------------------------------------------------------------------------
# AJUSTE [4/4]: Filtro defensivo de NA no grafico3
# Mesmo raciocínio do Ajuste 3 — proteção para futuros dados.
# ---------------------------------------------------------------------------

grafico3 <- df_integrado |>
  st_drop_geometry() |>
  filter(!is.na(subprefeitura), !is.na(valor), !is.na(status_valor)) |>
  ggplot(aes(y = reorder(subprefeitura, idrgp_alvo))) +
  geom_col(
...
## G4 – Dumbbell: IDRGP Real 2022 vs. 2025 por subprefeitura ------------------
#
# Substitui o gráfico de barras empilhadas.
# Mostra simultaneamente:
#   • Ponto 2022 (círculo claro) e ponto 2025 (círculo escuro): posição real
#   • Segmento entre eles: direção do movimento no ciclo
#   • Cor do segmento: melhorou (verde) ou piorou (laranja) em relação ao alvo
#   • Triângulo vermelho: IDRGP Alvo (linha de referência)
#
# Ordenação: pelo IDRGP Alvo (maior vulnerabilidade no topo), mantendo
# consistência visual com os demais gráficos do relatório.
# ---------------------------------------------------------------------------

# Prepara base do dumbbell com classificação de direção
df_dumbbell <- df_ciclo |>
  select(subprefeitura, idrgp_alvo, idrgp_real_2022, idrgp_real_2025) |>
  filter(
    !is.na(subprefeitura),
    !is.na(idrgp_real_2022),
    !is.na(idrgp_real_2025)
  ) |>
  mutate(
    # Diferença de posição relativa ao alvo: positivo = aproximou do alvo
    delta_alvo_2022 = abs(idrgp_real_2022 - idrgp_alvo),
    delta_alvo_2025 = abs(idrgp_real_2025 - idrgp_alvo),
    direcao = if_else(
      delta_alvo_2025 <= delta_alvo_2022,
      "Aproximou do alvo",
      "Distanciou do alvo"
    )
  )

# Cores do segmento (direção do movimento)
cores_direcao <- c(
  "Aproximou do alvo"   = "#0F6E56",   # verde escuro
  "Distanciou do alvo"  = "#d97a0f"    # laranja
)

grafico4 <- ggplot(df_dumbbell,
                   aes(y = reorder(subprefeitura, idrgp_alvo))) +

  # Segmento ligando 2022 a 2025 (colorido pela direção)
  geom_segment(
    aes(
      x    = idrgp_real_2022,
      xend = idrgp_real_2025,
      yend = reorder(subprefeitura, idrgp_alvo),
      color = direcao
    ),
    linewidth = 1.2,
    lineend   = "round"
  ...
  # Ponto 2022 — círculo claro com borda
  geom_point(
    aes(x = idrgp_real_2022, fill = "IDRGP Real 2022"),
    shape = 21, size = 3.2, stroke = 0.8,
    color = "#9BD7EA", fill = "#9BD7EA"
  ) +

  # Ponto 2025 — círculo escuro
  geom_point(
    aes(x = idrgp_real_2025, fill = "IDRGP Real 2025"),
    shape = 21, size = 3.2, stroke = 0.8,
    color = "#0B3C6D", fill = "#0B3C6D"
  ) +

  # Triângulo: IDRGP Alvo
  geom_point(
    aes(x = idrgp_alvo, shape = "IDRGP Alvo"),
    color = "#c0392b", size = 3.2
  ) +

  # Escalas
  scale_color_manual(
    values = cores_direcao,
    name   = "Movimento 2022→2025:"
  ) +
  scale_shape_manual(
    values = c("IDRGP Alvo" = 17),
    name   = NULL
  ) +

  # Legenda manual dos pontos (já que usamos fill fixo, não mapeado a aes)
  guides(
    color = guide_legend(
      order = 1,
    ...
    shape = guide_legend(
      order = 2,
      override.aes = list(color = "#c0392b", size = 3)
    )
  ) +

  # Eixo X em percentual com 1 casa decimal
  scale_x_continuous(
    labels = scales::label_percent(accuracy = 0.1)
  ) +

  # Anotação dos pontos 2022 e 2025 na legenda via caption
  labs(
    title    = "IDRGP 2022–2025 — Evolução do Gasto Real por Subprefeitura",
    subtitle = paste0(
      "Círculo claro: IDRGP Real 2022  |  Círculo escuro: IDRGP Real 2025  |  ",
      "Triângulo: IDRGP Alvo\n",
      "Segmento verde: aproximou do alvo no ciclo  |  Segmento laranja: distanciou do alvo"
    ),
    x       = "IDRGP Real (%)",
    y       = NULL,
    caption = paste0(
      caption_base, "\n",
      "Nota técnica: ordenação pelo IDRGP Alvo (maior vulnerabilidade no topo). ",
      "Direção avaliada pela distância absoluta ao IDRGP Alvo em 2022 vs. 2025."
    )
  ) +
...
## 11.3 Exporta gráficos como PNG (300 dpi) -----------------------------------
...
ggsave(file.path(pasta_saida, "grafico4_idrgp_dumbbell_2022_2025.png"),        grafico4,     width = 14, height = 12, dpi = 300)
...
## 11.5 Exporta shapefile e geopackage do IDRGP 2025 --------------------------
...
cat("  grafico4_idrgp_dumbbell_2022_2025.png     — IDRGP 2022→2025: dumbbell\n")
...
cat("Geoespacial:\n")
cat("  idrgp_2025.gpkg\n")
cat("  idrgp_ciclo_2022_2025.gpkg\n")
cat("  ", shp_nome, ".shp/.dbf/.prj\n\n")
